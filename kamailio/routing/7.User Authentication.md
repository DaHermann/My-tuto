...
* définir le paramètre load_credential pour charger la valeur de la colonne dans $avp(status)
        ...
        modparam("auth_db", "load_credentials", "$avp(status)=status")
            ...
            if (is_method("REGISTER")) {
                ...
                if ($au!=$tU) {
                    ...
                    if (from_uri==myself) { 
                        ...
                        #!endif
                        if ($fd!=$rd) {
                            sl_send_reply("403","Forbidden R-URI domain"); exit;
                        }
                    } else {
                        if ($au!=$fU) {
                            sl_send_reply("403","Forbidden auth ID");
...
Par conséquent, la relation entre les valeurs qui peuvent être dans le nom d'utilisateur d'authentification et de / vers
Le nom d'utilisateur peut être stocké dans la base de données.



## AUTHENTICATION USING A CUSTOM BACKEND (L'AUTHENTIFICATION À L'AIDE D'UN BACKEND PERSONNALISÉ)
...
La route [AUTH] pourrait ressembler à :
        ...
        route[AUTH] {
            if(is_method("REGISTER") || from_uri==myself) { 
                ...
                # authorization header is present - fetch the password 
                http_query("http://myserver.com/getpassword.php?username=$(au{s.escape.param})",
            "$var(password)");
                ...
                # user authenticated - remove auth header if(!is_method("REGISTER|PUBLISH"))
                consume_credentials();
            }
...
* s'il s'agit d'un enregistrement ou d'une demande d'un utilisateur se faisant passer pour local, vérifiez si les en-têtes d'autorisation figurent dans les demandes
...
du module utils, en le stockant dans $var(password)
...
* les dernières actions consistent à supprimer les en-têtes d'autorisation pour les demandes à transmettre et à s'assurer que l'appelant ou l'appelé est bien local, afin d'éviter qu'il ne soit utilisé comme relais ouvert
Vous pouvez en savoir plus sur les fonctions utilisées dans ce fichier de configuration à l'adresse suivante

• http://kamailio.org/docs/modules/4.2.x/modules/auth.html 
• http://kamailio.org/docs/modules/4.2.x/modules/utils.html
