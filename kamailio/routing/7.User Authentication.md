...
* set load_credential parameter to load the value of the column in $avp(status)
        
        modparam("auth_db", "load_credentials", "$avp(status)=status")
        
* in the configuration file, in route[AUTH], after the authentication check was ok, add the test for the $avp(status), the lines started with ‘+.’:
